...
def gen_files(tempfile=tempfile):
    ...
    """
    with open(tempfile) as f_in:
        for i in f_in.read().splitlines():
            directory, is_dir = i.split(',')
            if is_dir == 'True':
...
def diehard_pybites(files=None):
    ...
    challenge, user = map(list, zip(*[file.split('/') for file in files if file.split('/')[1] not in IGNORE]))
