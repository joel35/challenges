...
You can use this on a string as well as a list for example 'pybites'[0:2] gives 'py'.
...
def slice_and_dice(text: str = text) -> list:
    ...
    line_split = strip_text.split('\n')

    for line in line_split:
        strip_line = line.strip()
        if strip_line.islower():
            last_word = strip_line.split()[-1]
            strip_word = last_word.strip('.!')
